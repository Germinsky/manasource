...
uint256 hashGenesisBlock("0x440931c076a83d709fc434ebc647c9f757650205e3ba54ab6447f9f4826edf9c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Mana: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Mana Signed Message:\n";
...
{
    // Mana: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 3600;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Mana: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mana: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Mana: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x440931c076a83d709fc434ebc647c9f757650205e3ba54ab6447f9f4826edf9c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1407769987;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084633282;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ManaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ManaMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running Mana with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
